...
<% control Menu(1) %>
	<li class="$FirstLast<% if SiteMapPages %><% else %> noChildrenLI<% end_if %>">
		<% if SiteMapPages %><% if ShowAllPages %><% else %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel1" rel="$ID">+</a> <% end_if %><% end_if %>
		<a href="$Link" class="pageTitle SiteMapLevel1">$Title</a>
		<% if SiteMapPages %>
		<ul id="sublist{$ID}" class="SubListLevel2"><% control SiteMapPages %>
			<li class="$FirstLast<% if SiteMapPages %><% else %> noChildrenLI<% end_if %>">
				<% if SiteMapPages %><% if ShowAllPages %><% else %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel2" rel="$ID">+</a> <% end_if %><% end_if %>
				<a href="$Link" class="pageTitle SiteMapLevel2">$Title</a>
				<% if SiteMapPages %>
				<ul id="sublist{$ID}" class="SubListLevel3"><% control SiteMapPages %>
					<li class="$FirstLast<% if SiteMapPages %><% else %> noChildrenLI<% end_if %>">
						<% if SiteMapPages %><% if ShowAllPages %><% else %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel3" rel="$ID">+</a> <% end_if %><% end_if %>
						<a href="$Link" class="pageTitle SiteMapLevel3">$Title</a>
						<% if SiteMapPages %>
						<ul id="sublist{$ID}" class="SubListLevel4"><% control SiteMapPages %>
							<li class="$FirstLast<% if SiteMapPages %><% else %> noChildrenLI<% end_if %>">
								<% if SiteMapPages %><% if ShowAllPages %><% else %><a href="#" class="siteMapPageExpander SiteMapNodeImploded SiteMapLevel4" rel="$ID">+</a> <% end_if %><% end_if %>
