...
RegionPage::add_extension('RegionPage', 'GoogleMapLocationsDOD');
RegionPage::add_extension('RegionPage_Controller', 'GoogleMapLocationsDOD_Controller');
RegionPage_Controller::add_extension('RegionPage_Controller', 'GoogleMapLocationsDOD_Controller');
array_push(RegionBasePage::$extensions, 'GoogleMapLocationsDOD');

ListingPage::add_extension('ListingPage', 'GoogleMapLocationsDOD');
ListingPage::add_extension('ListingPage', 'GoogleMapLocationsDOD');
...
GoogleMap::setDefaultZoom(6);
GoogleMap::setGoogleMapWidth(512); //map width in pixels (ranges from around 100 to 900)
GoogleMap::setGoogleMapHeight(512); //map height in pixels (ranges from around 100 to 600)
...
GoogleMap::setAddDirections(true); //add directions finder to map pop-up windows
GoogleMap::setAddAddressFinder(true); //provide an address finder helping visitors to enter an address and search for it on the map
GoogleMap::setAddPointsToMap(true); //allow user to add their own points to the map using right-mouse-clicks
...
GoogleMap::setDropDownDivId("GmapDropDownID"); //ID for DIV of dropdown box with points in map (leave blank to remove)
...
GoogleMap::setAddCustomSearchForm(false); //add custom search
GoogleMap::setCustomSearchFormName(""); //name for custom form (or other html) on info-window"
GoogleMap::setCustomSearchFormHtml(""); //html for custom form (or other html) on info-window
...
/*  ADDRESS */
GoogleMap::setDefaultCountryCode("NZ"); //default country code for address searches (to narrow searches to one country) - examples include US or NZ
GoogleMap::setDefaultAddressText(", New Zealand"); //extra phrase added to the end of an address (e.g. New Zealand or United Kingdom)
