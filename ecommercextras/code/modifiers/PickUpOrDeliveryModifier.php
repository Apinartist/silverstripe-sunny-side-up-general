	...
	function LiveName() {
		...
		if(is_object($obj)) {
			...
			if($obj->ExplanationPageID) {
				...
				if($page) {
					$v .= '<div id="PickUpOrDeliveryModifierExplanationLink" class="externalLink"><a href="'.$page->Link().'">'.$page->Title.'</a></div>';
				}
			}
			return $v;
		}
				...
				if(($amount-0) == 0){
					...
					if( is_object($obj)) {
						...
						if(floatval($obj->MinimumOrderAmountForZeroRate) < floatval($amount)) {
							...
							$this->debugMessage .= "<hr />actual weight:".$weight." multiplier = ".$obj->WeightMultiplier;
							if($weight && $obj->WeightMultiplier ) {
								self::$actual_charges += $weight * $obj->WeightMultiplier;
