...
class ProductGroup extends Page {
	...
	function ProductsShowable($extraFilter = '', $permissions = array("Show All Products")) {
		...
		$filter = "{$bt}ShowInMenus{$bt} = 1";
		if($extraFilter) $filter .= " AND $extraFilter";
		$products = new DataObjectSet();

		$childProducts = DataObject::get('Product', "{$bt}ParentID{$bt} = $this->ID AND ".$filter);
	...
	function FeaturedProducts() {
		$bt = defined('DB::USE_ANSI_SQL') ? "\"" : "`";
		return $this->ProductsShowable("{$bt}FeaturedProduct{$bt} = 1", self::$featured_products_permissions);
	}
	...
	function NonFeaturedProducts() {
		$bt = defined('DB::USE_ANSI_SQL') ? "\"" : "`";
		return $this->ProductsShowable("{$bt}FeaturedProduct{$bt} = 0", self::$non_featured_products_permissions);
	}
	...
	function ChildGroups() {
		$bt = defined('DB::USE_ANSI_SQL') ? "\"" : "`";
		return DataObject::get('ProductGroup', "{$bt}ParentID{$bt} = '$this->ID' AND {$bt}ShowInMenus{$bt} = 1");
	}
